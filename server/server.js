const express = require('express');
const mongoose = require('mongoose');
const cors = require('cors');
const dotenv = require('dotenv');
dotenv.config();

const app = express();
app.use(cors({ origin: '*' }));
app.use(express.json());

app.use('/api/auth',    require('./routes/auth'));
app.use('/api/crops',   require('./routes/crops'));
app.use('/api/admin',   require('./routes/admin'));
app.use('/api/schemes', require('./routes/schemes'));
app.use('/api/msp',     require('./routes/msp'));
app.use('/api/forum',   require('./routes/forum'));
app.get('/', (_req, res) => res.json({ message: 'FarmConnect API v1.0' }));

mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/farmconnect')
  .then(async () => {
    console.log('Jk MongoDB Connected');
    const User = require('./models/User');
    if (!(await User.findOne({ role: 'admin' }))) {
      await User.create({
        name: 'Admin',
        email: process.env.ADMIN_EMAIL || 'admin@farmconnect.in',
        password: process.env.ADMIN_PASSWORD || 'Admin@1234',
        role: 'admin', isApproved: true,
      });
      console.log('Jk Admin seeded → admin@farmconnect.in / Admin@1234');
    }
  })
  .catch(err => console.error('❌ MongoDB:', err));

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => console.log(`Jk Port ${PORT} is running successfully`));
